...
async function routePost(handle: string, rkey: string): Promise<void> {
    try {
        announcePolite(`Carregando post de @${handle}...`);
        ...
        if (!modal || !container) {
            announceAssertive('Modal de posts não encontrado.');
            return;
        }

        if (titleEl) titleEl.textContent = `Thread de @${handle}`;
        container.innerHTML = '';
        state.currentPosts = [];

        const posts = res?.posts ?? [];
        if (posts.length === 0) {
            container.innerHTML = '<p>Post não encontrado ou sem conteúdo.</p>';
        } else {
...
async function routeList(handle: string, rkey: string): Promise<void> {
    try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        announceAssertive(`Erro ao abrir lista: ${msg}`);
    }
}

async function routeStarterPack(handle: string, rkey: string): Promise<void> {
    try {
        announcePolite(`Carregando starter pack de @${handle}...`);
        const did = await resolveHandleToDID(handle);
        ...
        dialog.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                ...
                <button type="button" id="btn-close-sp-modal" style="padding:4px 10px;">Fechar</button>
            </div>
            ${pack ? `
                <p><strong>${pack.name}</strong></p>
                ${pack.description ? `<p style="color:#aaa; font-size:0.9em;">${pack.description}</p>` : ''}
                <p style="font-size:0.85em;">Criado por: @${pack.creator}</p>
                <p style="font-size:0.8em; color:#777; word-break:break-all;">${uri}</p>
            ` : `<p>Starter pack de @${handle} (${rkey}).</p><p style="font-size:0.8em; color:#777; word-break:break-all;">${uri}</p>`}
        `;
        ...
        const msg = err instanceof Error ? err.message : String(err);
        announceAssertive(`Erro ao abrir starter pack: ${msg}`);
    }
}
...
function routeHashtag(tag: string): void {
    ...
    if (!searchInput || !btnSearch) {
        announceAssertive('Busca não disponível neste momento.');
        return;
    }
    ...
    switchTab('search');
    announcePolite(`Buscando posts com #${tag}...`);
    btnSearch.click();
}
